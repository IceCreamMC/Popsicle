...
import net.minecraft.popsicleclient.gui.screen.ButtonTextures;
...
import net.minecraft.popsicleclient.gui.widget.PressableWidget;
...
public class DoubleButton extends ClickableWidget implements Tickable {
    ...
    public void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawCenteredTextWithShadow(Minecraftpopsicleclient.getInstance().textRenderer, this.getMessage(), this.getX() + this.width / 2, this.getY() + (this.height - 8) / 2, (this.active ? 0xFFFFFF : 0xA0A0A0) | MathHelper.ceil(this.alpha * 255.0f) << 24);
    ...
    private void drawButton(DrawContext context, Text text, int x, boolean i) {
        ...
        context.drawCenteredTextWithShadow(Minecraftpopsicleclient.getInstance().textRenderer, text, x + this.height / 2, this.getY() + (this.height - 8) / 2, (this.active ? 0xFFFFFF : 0xA0A0A0) | MathHelper.ceil(this.alpha * 255.0f) << 24);
    }
    ...
    public void renderTooltip(DrawContext context, int mouseX, int mouseY) {
        if (this.tooltipDelay > 15 && Minecraftpopsicleclient.getInstance().currentScreen != null) {
...
//    }
    ...
    private void addValue(double value) {
        this.playDownSound(Minecraftpopsicleclient.getInstance().getSoundManager());
