...
import net.minecraft.popsicleclient.gui.screen.ingame.HandledScreen;
import net.minecraft.popsicleclient.render.DiffuseLighting;
import net.minecraft.popsicleclient.render.GameRenderer;
import net.minecraft.popsicleclient.render.VertexConsumerProvider;
import net.minecraft.popsicleclient.render.entity.EntityRenderDispatcher;
...
import org.icecream.icecream.popsicleclient.config.options.DoubleOption;
import org.icecream.icecream.popsicleclient.entity.Mob;
import org.icecream.icecream.popsicleclient.entity.Seat;
import org.icecream.icecream.popsicleclient.fake.FakePlayer;
...
import org.icecream.icecream.popsicleclient.mixin.accessor.AccessEntity;
import org.icecream.icecream.popsicleclient.mixin.accessor.AccessHoglin;
import org.icecream.icecream.popsicleclient.mixin.accessor.AccessMagmaCube;
import org.icecream.icecream.popsicleclient.mixin.accessor.AccessSlime;
...
public class MobScreen extends AbstractScreen {
    ...
    private final Text noPreview = Text.translatable("popsicleclient.options.no-preview");
    private final Text notImplemented = Text.translatable("popsicleclient.options.not-implemented");

    public MobScreen(Screen parent, Mob mob) {
        super(parent);
        this.mob = mob;
        this.subtitle = Text.translatable("popsicleclient.options.seat.title", mob.getType().getName());
    }
    ...
    public void init() {
        ...
        final Seat seat = popsicleclient.instance().getConfig().seats.getSeat(this.mob);
        ...
        if (this.popsicleclient == null || this.popsicleclient.player == null) {
            // we need a popsicleclient and player to draw a preview to...
            return;
        }

        this.fakePlayer = new FakePlayer(this.popsicleclient.world, this.popsicleclient.player);
        this.fakeEntity = this.mob.getType().create(this.popsicleclient.world);
    ...
    public void drawPreviewModel(FakePlayer player, Entity vehicle) {
        ...
        EntityRenderDispatcher renderer = Minecraftpopsicleclient.getInstance().getEntityRenderDispatcher();
        renderer.setRotation(quaternion2);
        renderer.setRenderShadows(false);
        VertexConsumerProvider.Immediate immediate = Minecraftpopsicleclient.getInstance().getBufferBuilders().getEntityVertexConsumers();

        //noinspection deprecation
        Minecraftpopsicleclient.getInstance().execute(() -> {
